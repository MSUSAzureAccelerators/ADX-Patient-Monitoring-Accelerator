...
$dtName=$(az dt list --resource-group $rgName --query [0].name --output tsv)
...
smartKneeBraceDevices=10
vitalPatchDevices=10
...
do
	...
	az dt twin create -n $dtName --dtmi "dtmi:PatientMonitoring:KneeBrace;1" --twin-id $deviceId --properties "{'PatientId': '${patient}'}"
	echo $deviceId
